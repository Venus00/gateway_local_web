...
const AddWidgetDialog = ({ id }: PropsType) => {
  ...
  const handleSubmit = () => {
    setAddWidgetMachine(id, {
      ...data,
      attributes: {
        ...data.attributes,
        timeframe: data.attributes?.timeframe || "1H",
      },
    });
    handleClose();
  };
  ...
  const Step2 = useCallback(() => {
    ...
    if (data.type === "button") return <ButtonOptions />;
    return null;
  }, [data.type]);
  ...
  return (
    ...
    >
      ...
      <DialogContent
        className={cn("max-w-2xl  max-h-[90%]  overflow-y-auto ", {
      ...
      >
        ...
        <DialogHeader className="font-semibold   flex items-center justify-center">
          <span className="first-letter:uppercase ">
            {
              step === 0
                ? `${widgetId === "new" ? "" : "edit widget"} `
                : `Modify widget ${data.type}`
