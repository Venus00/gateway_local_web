...
import { DeviceDto, DeviceEditDto } from "../device.dto";
...
interface PropsTaype {
  ...
  handleEdit: (device: DeviceEditDto) => void;
}

function DeviceTable({ devices, onDelete, handleEdit }: PropsTaype) {
  ...
  const table = useReactTable<DeviceDto>({
    data: devices,
    columns: columns(t, handleEdit, onDelete),
      ...
      <div className="">
        <div className="flex items-center py-4">
          <Input
            placeholder={
              isArabic
                ? "..." + t("table.filter") + " " + t("table.column.device")
                : t("table.filter") + " " + t("table.column.device") + "..."
            }
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="ml-auto">
                {t("table.columns")} <ChevronDownIcon className=" h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
