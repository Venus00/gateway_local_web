import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { BarChart, Shield, Zap } from "lucide-react";
import TextImageSection from "./TextImageSection";

export default function LandingPage() {
  return (
    <div
      className=""
      style={{ background: "linear-gradient(to left, #F2994A, #F2C94C)" }}
    >
      <TextImageSection />
      <section className="w-full hidden py-12 md:py-24 lg:py-32 xl:py-48">
        <div className="w-[100%] px-4 md:px-6">
          <div className="flex flex-col items-center space-y-4 text-center">
            <div className="space-y-2">
              <h1 className="text-3xl font-bold text-white tracking-tighter sm:text-4xl md:text-5xl lg:text-6xl/none">
                Welcome to Digital Senese
              </h1>
              <p className="mx-auto max-w-[700px] text-white  md:text-xl dark:text-gray-400">
                Emphasizing the connection between virtual and real devices.
              </p>
              <p className="mx-auto max-w-[700px] text-white  md:text-xl dark:text-gray-400">
                Highlighting the bridging of real devices with virtual ones.{" "}
              </p>
            </div>
            <div className="w-full max-w-sm space-y-2">
              <form className="flex space-x-2">
                <Input
                  className="max-w-lg flex-1 placeholder:text-white"
                  placeholder="Enter your email"
                  type="email"
                />
                <Button type="submit">Get Started</Button>
              </form>
              <p className="text-xs text-white dark:text-gray-400">
                Start your free trial. No credit card required.
              </p>
            </div>
          </div>
        </div>
      </section>

      <section className="w-full py-12 md:py-24 lg:py-32 bg-gray-100  relative dark:bg-gray-800">
        <div className="custom-shape-divider-top-1747842903">
          <svg
            data-name="Layer 1"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 1200 120"
            preserveAspectRatio="none"
          >
            <defs>
              <linearGradient
                id="customGradient"
                x1="0%"
                y1="0%"
                x2="100%"
                y2="0%"
              >
                <stop offset="0%" stopColor="#F2C94C" />
                <stop offset="100%" stopColor="#F2994A" />
              </linearGradient>
            </defs>
            <path
              d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z"
              opacity=".25"
              fill="url(#customGradient)"
            ></path>
            <path
              d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z"
              opacity=".5"
              fill="url(#customGradient)"
            ></path>
            <path
              d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z"
              fill="url(#customGradient)"
            ></path>
          </svg>
        </div>

        <div className="w-[100%] px-4 md:px-6">
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
            <div className="flex flex-col items-center space-y-4 text-center">
              <BarChart className="h-10 w-10 text-primary" />
              <h2 className="text-xl font-bold">Real-time Analytics</h2>
              <p className="text-gray-500 dark:text-gray-400">
                Monitor your factory's performance with advanced real-time
                analytics and reporting.
              </p>
            </div>
            <div className="flex flex-col items-center space-y-4 text-center">
              <Zap className="h-10 w-10 text-primary" />
              <h2 className="text-xl font-bold">Automated Workflows</h2>
              <p className="text-gray-500 dark:text-gray-400">
                Streamline your processes with intelligent automation and
                customizable workflows.
              </p>
            </div>
            <div className="flex flex-col items-center space-y-4 text-center">
              <Shield className="h-10 w-10 text-primary" />
              <h2 className="text-xl font-bold">Enhanced Security</h2>
              <p className="text-gray-500 dark:text-gray-400">
                Protect your data and operations with our state-of-the-art
                security measures.
              </p>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
